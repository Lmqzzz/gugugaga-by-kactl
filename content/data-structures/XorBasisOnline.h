...
struct xor_basis {
  ...
  bool add(T x, int id) {
    T mask = 0;
    for (auto &i : basis)
      if (umin(x, x ^ i.val)) mask ^= i.mask;
    if (x) {
      mask |= T(1) << basis.size();
      for (auto &i : basis)
        if (umin(i.val, i.val ^ x)) i.mask ^= mask;
  ...
  int get_the_same_high_bit(T bits, const vector<T> &val) {
    for (auto &i : basis)
      if (__builtin_popcount(val[i.id] & bits) & 1) return i.id;
    return -1;
  }
  T rebuild_and_delete(int id) {
    ...
    for (int i = 0; i < (int)basis.size(); ++i)
      if (basis[i].id == id) pos = i;
    for (int i = 0; i < (int)basis.size(); ++i)
      if (basis[i].mask >> pos & 1) {
...
struct xor_basis_online {
  ...
  int add(T x) {
    ...
    if (!x) return id;
    for (int i = mx; i >= 0; --i)
      if (!basises[i].empty() && basises[i].back().add(x, id)) {
